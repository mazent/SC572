...
/* #define DEBUG_PRINT */				/* Enable UART debug print messages */
// #define AUTOFOCUS 					/* Enable AUTOFOCUS function */
// #define STILL_IMAGE_CAPTURE_M2  		/* Enable UVC STILL IMAGE CAPTURE METHOD 2 */
// #define BRIGHTNESS_CONTROL	 		/* Enable the control of the brightness */
#define LOW_POWER_MODE  				/* Enable Low-power mode management */
#define FRAME_TIMER						/* Application Timer for frame timeout */

/* UVC application thread parameters. */
#define UVC_APP_THREAD_STACK           (0x1000) /* Stack size for the video streaming thread is 4 KB. 1000*/
#define UVC_APP_THREAD_PRIORITY        (8)      /* Priority for the video streaming thread is 8. */

#define UVC_APP_EP0_THREAD_STACK       (0x0800) /* Stack size for the UVC control request thread is 2 KB. 800*/
...
/* LP MODE event. This event indicates that the user has requested to enter in Low-Power mode.
 *  It should be cleared as soon as serviced by the firmware.
 */
#define CY_FX_LP_MODE_EVENT    (1 << 5)

/* FRONT CAM SELECTED event. This event flag indicates that the user has requested the use of the Front Camera.
 *  It should be cleared as soon as serviced by the firmware.
 */
#define CY_FX_FRONT_CAM_SEL_EVENT    (1 << 6)

/* REAR CAM SELECTED event. This event flag indicates that the user has requested the use of the Rear Camera.
 *  It should be cleared as soon as serviced by the firmware.
 */
#define CY_FX_REAR_CAM_SEL_EVENT    (1 << 7)
...
#define CY_FX_UVC_MAX_STILL_PROBE_SETTING			(11)			/* UVC 1.1 still probe control size */
#define CY_FX_UVC_MAX_STILL_PROBE_SETTING_ALIGNED	(32)			/* UVC 1.1 still probe control size (aligned to 32) */
#define CY_FX_UVC_STILL_IMAGE_TRIGGER_COUNT			(1)				/* UVC 1.1 still image trigger control size */
#define CY_FX_UVC_STILL_IMAGE_TRIGGER_ALIGNED		(16)

#define CY_FX_UVC_NORMAL_OPERATION		(0x00)
#define CY_FX_UVC_STILL_IMAGE			(0x01)
#define CY_FX_UVC_STILL_IMAGE_DEDICATED	(0x02)
#define CY_FX_UVC_ABORT_STILL_IMAGE		(0x03)
...
#define CY_FX_UVC_STILL_IMAGE_TRIGGER   (uint16_t)(0x0500)      /* wValue setting used to access STILL control. */
#define CY_FX_UVC_STILL_PROBE_CONTROL   (uint16_t)(0x0300)		/* Video STILL Probe Control Request */
#define CY_FX_UVC_STILL_COMMIT_CONTROL  (uint16_t)(0x0400)      /* Video STILL Commit Control Request */
